...
abstract contract CRISP is ERC721 {
    ...
    constructor(string memory _name, 
        ...
        targetEMS = PRBMathSD59x18.fromInt(1).div(PRBMathSD59x18.fromInt(1) - PRBMathSD59x18.fromInt(2).pow(-_targetBlocksPerSale.div(saleHalflife)));
    ...
    function getCurrentEMS() public view returns (int256 result) {
        int256 blockInterval = int(block.number - lastPurchaseBlock);
    ...
    function getQuote() public view returns (int256 result) {
        ...
        //decay price if we are past decay start block
            ...
            int256 decayInterval = int(block.number - priceDecayStartBlock).fromInt();
    ...
    function getNextStartingPrice(int256 lastPurchasePrice) public view returns (int256 result) {
        ...
        if (mismatchRatio > PRBMathSD59x18.fromInt(1)) {
            result = lastPurchasePrice.mul(PRBMathSD59x18.fromInt(1) + mismatchRatio.mul(priceSpeed));
        }
        else {
